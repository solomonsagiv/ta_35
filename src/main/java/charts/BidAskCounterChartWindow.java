...
public class BidAskCounterChartWindow extends JFrame {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;

	ChartUpdater updater1;
	ChartUpdater updater2;
	ChartUpdater updater3;
	...
	// Races series array
	XYSeries[] series2 = { new XYSeries("c") };

	XYSeries[] series3 = { new XYSeries("p") };
	...
	public static void main(String[] args) {
		BidAskCounterChartWindow chart = new BidAskCounterChartWindow(2000);
		chart.pack();
		chart.setVisible(true);
	}

	// Constructor
	public BidAskCounterChartWindow(int timeInSecondes) {
	...
	// Constructor
	public BidAskCounterChartWindow(int timeInSecondes, String strike) {
		setTitle(strike);
		this.strike = strike;
		init();
	}
	...
	private void init() {
		...
		chart2 = getChart("c" + strike, series2, green, blue);
		chart3 = getChart("p" + strike, series3, red, blue);
		...
		// Updaters
		updater1 = new ChartUpdater(series1, chart1, chart1.getXYPlot(), "index", 0.5, timeInSecondes);
		updater1.start();

		updater2 = new ChartUpdater(series2, chart2, chart2.getXYPlot(), "c" + strike, 1, timeInSecondes);
		updater2.start();

		updater3 = new ChartUpdater(series3, chart3, chart3.getXYPlot(), "p" + strike, 1, timeInSecondes);
		updater3.start();
	}
...
class ChartUpdater extends Thread {
	...
	// Constructor
	public ChartUpdater(XYSeries[] series, JFreeChart chart, XYPlot plot, String data, double margin,
			int timeInSecondes) {
		this.chart = chart;
		this.plot = plot;
		this.data = data;
		...
		if (data.contains("c")) {
			Option option = apiObject.getOptionsMonth().getOption(data);
			
			if (option.getBidAskCounterList().size() > 0) {
				for (int i = 0; i < option.getBidAskCounterList().size(); i++) {
					double price = option.getBidAskCounterList().get(i);
	...
	private void getDataFromApi(String data) throws IOException {
		...
		if (data.contains("c")) {
			prices[0] = apiObject.getOptionsMonth().getOption(data).getBidAskCounter();
		}
